...
data class PropertyData(
    ...
    @Expose @SerializedName("isFeatured") val isFeatured: Boolean?,
    ...
    override fun convertToDomainData(): PropertyCardModel {
        return PropertyCardModel(
            ...
            lowestDormPricePerNight = lowestDormPrice?.let { (lowestDormPrice.value + lowestDormPrice.currency) },
            lowestDormPrice = lowestDormPrice?.value?.toFloat() ?: 0f,
            currency = lowestDormPrice?.currency ?: lowestPrivatePrice?.currency ?: "",
            lowestPrivatePricePerNight = lowestPrivatePrice?.let { (lowestPrivatePrice.value + lowestPrivatePrice.currency) },
